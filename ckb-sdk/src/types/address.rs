...
impl AddressPayload {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        match self {
            AddressPayload::Short { index, args } => {
                let mut data = vec![0u8; 1 + args.len()];
                data[0] = (*index) as u8;
                data[1..21].copy_from_slice(args.as_ref());
                data
            }
            AddressPayload::Full {
                code_hash, args, ..
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        ...
        match ty {
            AddressType::Short => {
                if data.len() < 2 {
                    return Err(format!(
                        "Insufficient data length for short address: {}",
                        data.len()
                    ));
                }
                ...
                let raw_data = &data[2..];
                match index {
                    CodeHashIndex::Sighash | CodeHashIndex::Multisig => {
                        if raw_data.len() != 20 {
                            return Err(format!(
                                "Invalid data length for sighash/multisig address: {}",
                                data.len()
                            ));
                        }
                    }
                    CodeHashIndex::Acp => {
                        if raw_data.len() < 20 || raw_data.len() > 22 {
                            return Err(format!(
                                "Invalid data length for anyone-can-pay address: {}",
                                data.len()
                            ));
                        }
                    }
                }
                let args = Bytes::from(raw_data.to_vec());
                let payload = AddressPayload::Short { index, args };
...
mod test {
    ...
    fn test_short_address() {
        ...
        let payload =
            AddressPayload::new_multisig(&h160!("0x4fb2be2e5d0c1a3b8694f832350a33c1685d477a"));
        let address = Address::new(NetworkType::Mainnet, payload);
        assert_eq!(
            address.to_string(),
